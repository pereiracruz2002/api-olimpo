...
class Lib_apipagseguro{
  var $token = "B348949A90E54BBE980913F7F0C494A6";
  var $email = "de.akao@gmail.com";
  ...
  public function novaCompra($itens=array(), $dados_cliente=array()){
    ...
    $this->carrinho['senderEmail'] = $dados_cliente['email'];
    if(isset($dados_cliente['nome']) and $dados_cliente['nome'] and strstr($dados_cliente['nome'], ' '))
      $this->carrinho['senderName'] = abreviaString($dados_cliente['nome'], 50, '');

    if(isset($dados_cliente['cidade']) and $dados_cliente['cidade'])
      $this->carrinho['shippingAddressCity'] = abreviaString($dados_cliente['cidade'], 80, '');

    if(isset($dados_cliente['estado']) and $dados_cliente['estado'])
      $this->carrinho['shippingAddressState'] = strtoupper(abreviaString($dados_cliente['estado'], 2, ''));

    if(isset($dados_cliente['endereco']) and $dados_cliente['endereco'])
      $this->carrinho['shippingAddressStreet'] = abreviaString($dados_cliente['endereco'], 80, '');

    if(isset($dados_cliente['bairro']) and $dados_cliente['bairro'])
      $this->carrinho['shippingAddressDistrict'] = abreviaString($dados_cliente['bairro'], 80, '');


    if(isset($dados_cliente['numero']) and $dados_cliente['numero']){
      $dados_cliente['numero'] = (int) $dados_cliente['numero'];
      $this->carrinho['shippingAddressNumber'] = abreviaString($dados_cliente['numero'], 20, '');
    }

    if(isset($dados_cliente['complemento']) and $dados_cliente['complemento'])
      $this->carrinho['shippingAddressComplement'] = abreviaString($dados_cliente['complemento'], 40, '');

    if(isset($dados_cliente['cep'])){
      $dados_cliente['cep'] = str_replace('-', '', $dados_cliente['cep']);
      if($dados_cliente['cep'] and strlen($dados_cliente['cep']) == 8)
        $this->carrinho['shippingAddressPostalCode'] = $dados_cliente['cep'];
     }
  ...
  public function notificationPost(){
    ...
    if(!$erro){
      $this->ipn_data['Referencia'] = (string) $objXml->reference;
      $this->ipn_data['TransacaoID'] = (string) $objXml->code;
      $status = (string) $objXml->status;
      $this->ipn_data['StatusTransacao'] = pagseguro_status($status);
      return true;
    }
    else
      return false;
  ...
  public function directCreditCard($itens=array(), $dados_cliente=array()) 
  {
      ...
      $this->carrinho['installmentValue'] = number_format($this->carrinho['installmentValue'], 2, '.','');
      $this->carrinho['creditCardHolderBirthDate'] = date('d/m/Y', strtotime($this->carrinho['creditCardHolderBirthDate']));
      $this->carrinho['creditCardHolderCPF'] = preg_replace('/[^\d\s]/', '', $this->carrinho['creditCardHolderCPF']);
      $this->carrinho['creditCardHolderName'] = utf8_decode(abreviaString($this->carrinho['creditCardHolderName'], 50, ''));
      $this->carrinho['creditCardHolderPhone'] = preg_replace('/[^\d\s]/', '', $this->carrinho['creditCardHolderPhone']);

      $this->carrinho['billingAddressPostalCode'] = preg_replace('/[^\d\s]/', '', $dados_cliente['cep']);
      $this->carrinho['billingAddressStreet'] = utf8_decode($dados_cliente['endereco']);
      $dados_cliente['numero'] = (int) $dados_cliente['numero'];
      $this->carrinho['billingAddressNumber'] = $dados_cliente['numero'];
      if(isset($dados_cliente['complemento']) and $dados_cliente['complemento'])
          $this->carrinho['billingAddressComplement'] = utf8_decode($dados_cliente['complemento']);

      $this->carrinho['billingAddressDistrict'] = utf8_decode(abreviaString($dados_cliente['bairro'], 60, ''));
      $this->carrinho['billingAddressCity'] = utf8_decode(abreviaString($dados_cliente['cidade'], 60, ''));
      $this->carrinho['billingAddressState'] = utf8_decode(abreviaString($dados_cliente['estado'], 2, ''));
      $this->carrinho['billingAddressCountry'] = 'BRA';

      $dados_cliente['cpf'] = $this->carrinho['creditCardHolderCPF'];
      $dados_cliente['nome'] = $this->carrinho['creditCardHolderName'];
      $dados_cliente['telefone'] = '('.$this->carrinho['creditCardHolderAreaCode'].') '.$this->carrinho['creditCardHolderPhone'];
  ...
  private function directRequest($dados_cliente) 
  {
      ...
      if(ENVIRONMENT == 'development'){
          $dados_cliente['email'] = 'c96576168997972118179@sandbox.pagseguro.com.br';
      }
      ...
      $this->carrinho['senderEmail'] = abreviaString($dados_cliente['email'], 60, '');
      $this->carrinho['senderName'] = utf8_decode(abreviaString($dados_cliente['nome'], 50, ''));
      $this->carrinho['senderCPF'] = preg_replace('/[^\d\s]/', '', $dados_cliente['cpf']);
      $this->carrinho['senderAreaCode'] = substr($dados_cliente['telefone'], 1, 2);
      $this->carrinho['senderPhone'] = preg_replace('/[^\d\s]/', '', substr($dados_cliente['telefone'], 5));
      $this->carrinho['shippingAddressCountry'] = 'BRA';
      $this->carrinho['shippingAddressState'] = abreviaString($dados_cliente['estado'], 2, '');
      $this->carrinho['shippingAddressCity'] = utf8_decode(abreviaString($dados_cliente['cidade'], 60, ''));
      $this->carrinho['shippingAddressPostalCode'] = preg_replace('/[^\d\s]/', '', $dados_cliente['cep']);
      $this->carrinho['shippingAddressDistrict'] = utf8_decode(abreviaString($dados_cliente['bairro'], 60, ''));
      $this->carrinho['shippingAddressStreet'] = utf8_decode(abreviaString($dados_cliente['endereco'], 80, ''));
      $dados_cliente['numero'] = (int) $dados_cliente['numero'];
      $this->carrinho['shippingAddressNumber'] = abreviaString($dados_cliente['numero'], 20, '');
      if(isset($dados_cliente['complemento']) and $dados_cliente['complemento'])
          $this->carrinho['shippingAddressComplement'] = utf8_decode(abreviaString($dados_cliente['complemento'], 40, ''));
